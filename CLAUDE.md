...
| 步骤 | 脚本                    | 输入                  | 输出             | 耗时  |
| ---- | ----------------------- | --------------------- | ---------------- | ----- |
| ①   | `scan_funds.py`       | AKShare 全量基金      | 5 个分类 JSON    | ~30s  |
...
**增量更新**（每工作日 08:30 / 17:30 / 22:30 北京时间）跑 ③→④→②（fill_missing → refresh_purchase → enrich_data）。
...
| `active`         | 美股主动基金（白名单精选）      | 主动   |
| `global_other`   | 其他全球型 QDII                 | 主动   |
